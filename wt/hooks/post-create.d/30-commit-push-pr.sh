#!/usr/bin/env bash
# Commit the scaffold, push the branch, open a draft PR. Lives in a hook rather
# than in the binary so there is no `gh` dependency in Rust and the commit
# message or PR body can change without a rebuild.
set -euo pipefail
